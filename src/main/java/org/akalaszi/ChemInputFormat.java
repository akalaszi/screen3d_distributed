...
public class ChemInputFormat extends InputFormat<String, String> {
	...
	/**
	 * based on {@link FileInputFormat} List input directories. Subclasses may
	 * override to, e.g., select only files matching a regular expression.
	 * 
	 * @param job
	...
	public List<InputSplit> getSplits(JobContext job) throws IOException,
		...
		for (FileStatus file : files) {
			...
			FSDataInputStream istream = path.getFileSystem(
					job.getConfiguration()).open(path);

			MRecordReader mr = MFileFormatUtil
					.createRecordReader(istream, null);
			MRecord record = null;
			...
			try {
				while ((record = mr.nextRecord()) != null) {
					...
					if (!toCurrentSplit.isEmpty()
							&& processedRecordCount % DEFAULT_RECORDS_PER_SPLIT == 0) {
