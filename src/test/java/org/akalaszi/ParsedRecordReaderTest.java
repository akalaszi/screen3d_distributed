...
public class ParsedRecordReaderTest {
    ...
    public void test() throws IOException, InterruptedException {
        ...
        Assert.assertEquals("2", prr.getCurrentKey().toString());
        Assert.assertEquals("val2", prr.getCurrentValue().toString());
        Assert.assertEquals(0.66, prr.getProgress(), 1e-2);
        Assert.assertTrue(prr.nextKeyValue());

        Assert.assertEquals("3", prr.getCurrentKey().toString());
        Assert.assertEquals("val3", prr.getCurrentValue().toString());
